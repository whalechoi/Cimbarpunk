...
- Linux：GCC、构建工具及 Qt/XCB/OpenGL 开发包。
...
脚本要求 ZIP 已校验且已解压到 `SourceDirectory`；它不会删除或覆盖源码/构建目录。构建前应保证目标路径没有另一套不完整 Qt。
...
Linux M1 只使用 Release 预设；`scripts/provision-qt-source-linux.sh ABSOLUTE_PREFIX ABSOLUTE_WORKDIR` 下载、校验并构建同一官方源码的 Release 套件。若以后需要 `linux-debug`，应在独立前缀构建 Debug Qt，不要把不同 ABI 的库混入同一非多配置 Unix 前缀。
...
$vcpkg = 'C:\src\vcpkg'
pwsh -File scripts/configure-windows.ps1 -QtRoot $qt -VcpkgRoot $vcpkg -Preset windows-debug
cmake --build --preset windows-debug --target cimbarpunk cimbarpunk_tests
ctest --preset windows-debug --output-on-failure
```

脚本可从普通 PowerShell 启动：它通过 `vswhere` 导入并验证 VS 2022 x64 开发环境，检查 CMake、Ninja、精确 Qt、Svg/qsvgicon 和 vcpkg。若 OpenCV 首次配置只因 pkg-config 路径解析失败，脚本会查找已安装的 `pkgconf.exe`，只携带 `PKG_CONFIG` 重试一次；其他失败不会被掩盖。
...
输出目录是 `out/install/windows-release`。验证脚本会重新配置、构建应用/测试/播放器、运行 CTest、安装，再调用 `windeployqt --release --qmldir src/selection/qml --include-plugins qsvgicon`，并检查 Qt Svg、`qsvgicon`、平台插件和许可证文件。
...
out\build\windows-release\tests\manual\frame_player\cimbarpunk_frame_player.exe tests\fixtures\cimbar
```

它按 `manifest.json` 的 `orderedFrames` 顺序在黑色背景中央以 10 FPS 循环显示 PNG；按 `Esc` 退出。它只用于验证动态桌面捕获，不是产品组件。

## 源码提供

分发 Cimbarpunk 二进制时，应同时提供与该二进制完全对应的本项目源代码、构建脚本、vcpkg baseline、固定 `libcimbar` 子模块内容和 GPL-3.0-only 许可证；Qt LGPL、libcimbar MPL 及其他第三方义务仍分别适用。发布物中的 `share/licenses/cimbarpunk` 包含项目许可证、第三方声明和固定 libcimbar 的完整 MPL-2.0 文本。
